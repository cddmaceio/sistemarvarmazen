...
const colaboradoresRoutes = new Hono<{ Bindings: Env }>();
...
colaboradoresRoutes.get('/individual-report', async (c) => {
  ...
  try {
    ...
    const lancamentosDetalhados = lancamentosAprovados.map(lancamento => {
      let atividadesProcessadas: any[] = [];
      let kpisProcessados: any[] = [];
      ...
      if (lancamento.kpis_atingidos) {
        try {
          ...
          if (Array.isArray(kpis)) {
            // Verificar se são strings (formato antigo) ou objetos (formato novo)
            if (kpis.length > 0 && typeof kpis[0] === 'string') {
              // Formato antigo: array de strings com nomes dos KPIs
              // Todos os KPIs no array são considerados atingidos
              kpisProcessados = kpis.map(nomeKpi => ({
                nome: nomeKpi,
                valor: 0, // Valor não disponível no formato antigo
                atingido: true // Se está no array, foi atingido
              }));
            } else {
              ...
              kpisProcessados = kpis.map(kpi => ({
                ...
                valor: kpi.valor || 0,
                atingido: kpi.atingido !== undefined ? kpi.atingido : true
              }));
            }
          }
        } catch (e) {
    ...
    const lancamentosFormatados = lancamentosDetalhados.map(lancamento => {
      ...
      return {
        data: dataFormatada,
        diaSemana: diaSemana,
        bonus_kpis: parseFloat(lancamento.valorKPIs) || 0, // Adicionar campo bonus_kpis
