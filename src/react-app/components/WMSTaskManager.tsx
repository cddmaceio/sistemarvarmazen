...
interface WMSTaskManagerProps {
  selectedOperator: string;
  onOperatorChange: (operator: string) => void;
  availableOperators: string[];
  onCalculateProductivity: (data: { nome_operador: string; valid_tasks_count: number; data_referencia?: string }) => void;
  onOperatorsUpdate?: (operators: string[]) => void;
...
const WMSTaskManager: React.FC<WMSTaskManagerProps> = ({
  ...
  const handleFileSelect = (selectedFile: File | null) => {
    ...
    reader.onload = (e) => {
      setTimeout(() => {
        processData(e.target?.result as string);
  ...
  const processData = (csvContent: string) => {
    try {
  ...
  const analyzeData = () => {
    ...
    const dateFilteredData = csvData.filter(row => {
      ...
      if (!dataAssociacao) return false;
    ...
    const finalStats: Record<string, UserStats> = {};
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="mt-6">
              <button
                ...
                disabled={!userStats[currentOperator] || !selectedDate}
