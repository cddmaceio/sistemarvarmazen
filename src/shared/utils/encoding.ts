 ...
 * - ç becomes Ã§
 */
export function convertToDbFormat(text: string): string {
  return text
    .replace(/é/g, 'Ã©')
    .replace(/ã/g, 'Ã£')
    .replace(/ç/g, 'Ã§')
 ...
 */
export function convertFromDbFormat(text: string): string {
  return text
    .replace(/Ã©/g, 'é')
    .replace(/Ã£/g, 'ã')
    .replace(/Ã§/g, 'ç')
 ...
 */
export const FUNCAO_UI_TO_DB: Record<string, string> = {
 ...
 */
export const FUNCAO_DB_TO_UI: Record<string, string> = {
