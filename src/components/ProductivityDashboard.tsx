...
const ProductivityDashboard: React.FC = () => {
  ...
  const turnoData: TurnoData[] = productivityData.length > 0 ? productivityData.reduce((acc, item) => {
  ...
  const funcaoData: FuncaoData[] = productivityData.length > 0 ? productivityData.reduce((acc, item) => {
  ...
  const areaChartData = productivityData.length > 0 ? productivityData.reduce((acc, item) => {
    ...
    return acc;
  }, [] as Array<{funcao: string; produtividade: number; eficiencia: number; colaboradores: number}>) : [];
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    labelLine={false}
                    label={({ funcao, colaboradores }) => `${funcao}: ${colaboradores}`}
                    outerRadius={80}
                    fill="#8884d8"
                    dataKey="colaboradores"
                  >
                    {funcaoData.map((_, index) => (
                  ...
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      <span className={`font-semibold ${
                        item.produtividade >= item.meta ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {item.produtividade}%
                      </span>
                    </td>
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      <span className={`font-semibold ${
                        item.eficiencia >= 100 ? 'text-green-600' : 'text-orange-600'
                      }`}>
                        {item.eficiencia}%
                      </span>
                    </td>
            ...
            <div className="overflow-x-auto">
              {monthlyEarningsData.length > 0 ? (
                <table className="w-full border-collapse border border-gray-300">
                  ...
                  <tbody>
                    {monthlyEarningsData.map((colaborador, index) => (
                      <tr key={colaborador.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
