...
function isOperatorMatch(taskOperator, selectedOperator) {
    if (!taskOperator || !selectedOperator)
        return false;
...
async function processBulkTasks(c, nome_operador, rawTasks) {
    try {
        const supabase = getSupabase(c.env);
...
async function processBulkTasksAll(c, rawTasks) {
    try {
        const supabase = getSupabase(c.env);
...
function parseCSVStreaming(content) {
