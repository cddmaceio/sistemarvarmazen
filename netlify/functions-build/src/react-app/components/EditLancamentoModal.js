...
export default function EditLancamentoModal({ open, onClose, lancamento, onSave }) {
    ...
    return (<Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={formData.funcao} onChange={(e) => setFormData(prev => ({ ...prev, funcao: e.target.value }))}>
            ...
            <div className="space-y-4">
              {formData.funcao !== 'Ajudante de Armazém' && formData.funcao !== 'Operador de Empilhadeira' && (<>
                  <div>
                    ...
                    <Select value={formData.nome_atividade || ''} onChange={(e) => setFormData(prev => ({ ...prev, nome_atividade: e.target.value }))}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input type="number" value={formData.quantidade_produzida || ''} onChange={(e) => setFormData(prev => ({ ...prev, quantidade_produzida: Number(e.target.value) }))}/>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Tempo (horas)</label>
                      <Input type="number" step="0.1" value={formData.tempo_horas || ''} onChange={(e) => setFormData(prev => ({ ...prev, tempo_horas: Number(e.target.value) }))}/>
                    </div>
                  </div>
                </>)}
              ...
              {formData.funcao === 'Operador de Empilhadeira' && (<>
                  <div>
                    ...
                    <Input value={formData.nome_operador || ''} onChange={(e) => setFormData(prev => ({ ...prev, nome_operador: e.target.value }))}/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Tarefas Válidas</label>
                    <Input type="number" value={formData.valid_tasks_count || ''} onChange={(e) => setFormData(prev => ({ ...prev, valid_tasks_count: Number(e.target.value) }))}/>
                  </div>
                </>)}
            </div>
          </div>
          ...
          {formData.funcao === 'Ajudante de Armazém' && (<Card>
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                {multipleActivities.map((activity, index) => (<div key={index} className="grid grid-cols-4 gap-4 p-4 border rounded-lg">
                    <Select value={activity.nome_atividade} onChange={(e) => updateMultipleActivity(index, 'nome_atividade', e.target.value)}>
                      <option value="">Selecione atividade</option>
                      {activityNames.map((act) => (<option key={act.nome_atividade} value={act.nome_atividade}>
                          {act.nome_atividade}
                        </option>))}
                    </Select>
                    <Input type="number" placeholder="Quantidade" value={activity.quantidade_produzida || ''} onChange={(e) => updateMultipleActivity(index, 'quantidade_produzida', Number(e.target.value))}/>
                    <Input type="number" step="0.1" placeholder="Tempo (h)" value={activity.tempo_horas || ''} onChange={(e) => updateMultipleActivity(index, 'tempo_horas', Number(e.target.value))}/>
                    <Button variant="destructive" size="sm" onClick={() => removeMultipleActivity(index)}>
                      <X className="h-4 w-4"/>
                    </Button>
                  </div>))}
          ...
          <div>
            ...
            <Input type="number" step="0.01" value={formData.input_adicional || ''} onChange={(e) => setFormData(prev => ({ ...prev, input_adicional: Number(e.target.value) }))} placeholder="R$ 0,00"/>
          </div>
          ...
          <div>
            ...
            <Input value={editObservacoes} onChange={(e) => setEditObservacoes(e.target.value)} placeholder="Justifique as alterações realizadas..."/>
          </div>
          ...
          {/* Results Preview */}
          {result && hasCalculated && (<Card className="bg-green-50 border-green-200">
              <CardHeader>
          ...
          <div className="flex space-x-3 pt-4 border-t">
            ...
            <Button onClick={handleSave} disabled={!result || !hasCalculated} className="flex-1 bg-green-600 hover:bg-green-700">
