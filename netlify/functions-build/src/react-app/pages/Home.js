...
export default function Home() {
    ...
    const [selectedOperator, setSelectedOperator] = useState(user?.nome || '');
    const [wmsReferenceDate, setWmsReferenceDate] = useState('');
    ...
    useEffect(() => {
        ...
        if (userTurno && !isAdmin) {
            setFormData(prev => ({ ...prev, turno: userTurno }));
        }
    }, [userTurno, isAdmin]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100">
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto">
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select value={formData.funcao} onChange={(e) => handleInputChange('funcao', e.target.value)} placeholder="Função definida pelo usuário" disabled={true}>
                  ...
                  {isAjudanteArmazem && (<div className="space-y-4 p-4 bg-amber-50 rounded-lg border border-amber-200">
                      <div className="flex items-center justify-between">
                        ...
                        <Button type="button" variant="outline" size="sm" onClick={addActivity} className="text-amber-600 border-amber-300 hover:bg-amber-100">
                      ...
                      {multipleActivities.map((activity, index) => (<div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-3 p-3 bg-white rounded-lg border">
                          <div className="space-y-1">
                            ...
                            <Select value={activity.nome_atividade} onChange={(e) => updateActivity(index, 'nome_atividade', e.target.value)} placeholder="Selecione" disabled={activityNamesLoading}>
                          ...
                          <div className="space-y-1">
                            ...
                            <Input type="number" min="0" step="1" value={activity.quantidade_produzida || ''} onChange={(e) => updateActivity(index, 'quantidade_produzida', parseInt(e.target.value) || 0)} placeholder="Ex: 83"/>
                          </div>
                          <div className="space-y-1">
                            <label className="text-xs font-medium text-gray-600">Tempo (h)</label>
                            <Input type="number" min="0.1" step="0.1" value={activity.tempo_horas || ''} onChange={(e) => updateActivity(index, 'tempo_horas', parseFloat(e.target.value) || 0)} placeholder="Ex: 3"/>
                          </div>
                          <div className="flex items-end">
                            {multipleActivities.length > 1 && (<Button type="button" variant="destructive" size="sm" onClick={() => removeActivity(index)} className="w-full">
                  ...
                  {!isAjudanteArmazem && !isOperadorEmpilhadeira && (<>
                      <div className="space-y-2">
                        ...
                        <Select value={formData.nome_atividade || ''} onChange={(e) => handleInputChange('nome_atividade', e.target.value)} placeholder="Selecione a atividade" disabled={activityNamesLoading}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Input type="number" min="0" step="1" value={formData.quantidade_produzida || ''} onChange={(e) => handleInputChange('quantidade_produzida', parseInt(e.target.value) || 0)} placeholder="Ex: 83"/>
                        </div>

                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-700">Tempo Trabalhado (horas)</label>
                          <Input type="number" min="0.1" step="0.1" value={formData.tempo_horas || ''} onChange={(e) => handleInputChange('tempo_horas', parseFloat(e.target.value) || 0)} placeholder="Ex: 3"/>
                        </div>
                      </div>
                    </>)}

                  {/* WMS Task Manager for Operador de Empilhadeira */}
                  {isOperadorEmpilhadeira && (<WMSTaskManager selectedOperator={selectedOperator} onOperatorChange={setSelectedOperator} availableOperators={availableOperators} onOperatorsUpdate={setAvailableOperators} onCalculateProductivity={(data) => {
                ...
                calculate(calculatorData);
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" min="0" step="0.01" value={formData.input_adicional || ''} onChange={(e) => handleInputChange('input_adicional', parseFloat(e.target.value) || 0)} placeholder="Ex: 15.50"/>
                  </div>
                  ...
                  {availableKPIs.length > 0 && (<div className="space-y-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    : 'bg-white border-gray-200 hover:border-blue-300'}`} onClick={() => toggleKPI(kpi.nome_kpi)}>
                            <div className="flex items-center justify-between">
                              ...
                              {selectedKPIs.includes(kpi.nome_kpi) && (<CheckCircle className="h-5 w-5 text-green-600"/>)}
                            </div>
                          </div>))}
                      </div>
                    </div>)}

                  <Button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900" disabled={calculating || !formData.funcao}>
                ...
                {result && (<div className="mt-6 p-6 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg border border-green-200">
                    ...
                    <div className="space-y-3">
                      ...
                      {result?.kpisAtingidos && result.kpisAtingidos.length > 0 && (<div className="mt-4">
                          <p className="text-sm text-gray-600 mb-2">KPIs Atingidos:</p>
                          <div className="flex flex-wrap gap-2 mb-3">
                            {result?.kpisAtingidos?.map((kpi, index) => (<span key={index} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                                {kpi}
                              </span>))}
                          </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-green-200">
                      <Button onClick={handleOpenLancamento} className="w-full bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900 text-white">
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Data do Lançamento
                            {isOperadorEmpilhadeira && (<span className="text-xs text-blue-600 ml-2">
                                (Travada pela data de referência das tarefas)
                              </span>)}
                          </label>
                          <Input type="date" value={dataLancamento} onChange={async (e) => {
            ...
            }} max={new Date().toISOString().split('T')[0]} disabled={isOperadorEmpilhadeira} className={isOperadorEmpilhadeira ? 'bg-gray-100 cursor-not-allowed' : ''}/>
                          {isOperadorEmpilhadeira && !wmsReferenceDate && (<p className="text-xs text-amber-600 mt-1">
                ...
                setDataLancamento('');
                          ...
                          <Button onClick={handleLancarProdutividade} disabled={!dataLancamento || lancando} className="flex-1 bg-green-600 hover:bg-green-700 text-white">
