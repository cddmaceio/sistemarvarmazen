      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="space-y-8">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input type="date" value={filtros.periodo_inicio} onChange={(e) => handleFilterChange('periodo_inicio', e.target.value)}/>
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700 flex items-center">
                      <Calendar className="h-4 w-4 mr-1"/>
                    ...
                    <Input type="date" value={filtros.periodo_fim} onChange={(e) => handleFilterChange('periodo_fim', e.target.value)}/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={filtros.funcao || ''} onChange={(e) => handleFilterChange('funcao', e.target.value || undefined)}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={filtros.colaborador_id || ''} onChange={(e) => handleFilterChange('colaborador_id', e.target.value ? parseInt(e.target.value) : undefined)}>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filtros.status} onChange={(e) => handleFilterChange('status', e.target.value)}>
                ...
                <div className="flex flex-wrap gap-3 pt-4 border-t">
                  <Button onClick={gerarPreview} disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white">
                    <Eye className="h-4 w-4 mr-2"/>
                    {loading ? 'Carregando...' : 'Visualizar Preview'}
                  </Button>
                  
                  {showPreview && preview && (<Button onClick={exportarDados} disabled={exporting || !preview.dados.length} className="bg-green-600 hover:bg-green-700 text-white">
                      <Download className="h-4 w-4 mr-2"/>
                      {exporting ? 'Exportando...' : 'Exportar Dados'}
                    </Button>)}
                  
                  <Button onClick={limparFiltros} variant="outline" className="border-gray-300 text-gray-700 hover:bg-gray-50">
            ...
            {/* Preview dos Dados */}
            {showPreview && preview && (<Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                ...
                <CardContent>
                  {preview.dados.length === 0 ? (<div className="text-center py-8 text-gray-500">
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {preview.dados.slice(0, 10).map((item, index) => (<TableRow key={index}>
            ...
            <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
              ...
              <CardContent>
                {logs.length === 0 ? (<div className="text-center py-8 text-gray-500">
                    ...
                    <TableBody>
                      {logs.slice(0, 10).map((log) => (<TableRow key={log.id}>
