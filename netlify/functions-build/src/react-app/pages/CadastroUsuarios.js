...
import { FUNCOES_UI_FORMAT, TURNOS_UI_FORMAT, FUNCAO_UI_TO_DB, TURNO_UI_TO_DB, FUNCAO_DB_TO_UI, TURNO_DB_TO_UI } from '@/shared/utils/encoding';
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {filteredUsers.map((user) => (<TableRow key={user.id}>
                            ...
                            <TableCell>
                              <button onClick={() => toggleUserStatus(user)} className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium transition-colors ${(user.status_usuario === 'ativo' || user.is_active)
                    ? 'bg-green-100 text-green-800 hover:bg-green-200'
                    : 'bg-red-100 text-red-800 hover:bg-red-200'}`}>
                                <span className={`w-2 h-2 rounded-full ${(user.status_usuario === 'ativo' || user.is_active) ? 'bg-green-600' : 'bg-red-600'}`}></span>
                                <span>{(user.status_usuario === 'ativo' || user.is_active) ? 'Ativo' : 'Inativo'}</span>
                              </button>
                            </TableCell>
                            <TableCell>
                              <div className="flex items-center space-x-2">
                                <Button variant="outline" size="sm" onClick={() => openViewDialog(user)} className="text-blue-600 hover:text-blue-800" title="Visualizar">
                                  <Eye className="h-3 w-3"/>
                                </Button>
                                <Button variant="outline" size="sm" onClick={() => openEditDialog(user)} className="text-green-600 hover:text-green-800" title="Editar">
                                  <Edit className="h-3 w-3"/>
                                </Button>
                                <Button variant="destructive" size="sm" onClick={() => handleDelete(user)} className="text-red-600 hover:text-red-800" title="Excluir">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={formData.nome} onChange={(e) => setFormData(prev => ({ ...prev, nome: e.target.value }))} placeholder="Nome completo do usuário" required/>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={formData.cpf} onChange={(e) => handleCPFChange(e.target.value)} placeholder="000.000.000-00" maxLength={14} required/>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="date" value={formData.data_nascimento} onChange={(e) => setFormData(prev => ({ ...prev, data_nascimento: e.target.value }))} required/>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.funcao} onChange={(e) => setFormData(prev => ({ ...prev, funcao: e.target.value }))} required>
                      {FUNCOES_SISTEMA.map(funcao => (<option key={funcao} value={funcao}>{funcao}</option>))}
                    </Select>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-4">
                      <label className="flex items-center space-x-2">
                        <input type="radio" name="tipo_usuario" value="colaborador" checked={formData.tipo_usuario === 'colaborador'} onChange={(e) => setFormData(prev => ({ ...prev, tipo_usuario: e.target.value }))}/>
                        <User className="h-4 w-4 text-gray-600"/>
                        <span className="text-sm">User</span>
                      </label>
                      <label className="flex items-center space-x-2">
                        <input type="radio" name="tipo_usuario" value="administrador" checked={formData.tipo_usuario === 'administrador'} onChange={(e) => setFormData(prev => ({ ...prev, tipo_usuario: e.target.value }))}/>
                        <Shield className="h-4 w-4 text-purple-600"/>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-4">
                      <label className="flex items-center space-x-2">
                        <input type="radio" name="status_usuario" value="ativo" checked={formData.status_usuario === 'ativo'} onChange={(e) => setFormData(prev => ({ ...prev, status_usuario: e.target.value }))}/>
                        <span className="text-sm">🟢 Ativo</span>
                      </label>
                      <label className="flex items-center space-x-2">
                        <input type="radio" name="status_usuario" value="inativo" checked={formData.status_usuario === 'inativo'} onChange={(e) => setFormData(prev => ({ ...prev, status_usuario: e.target.value }))}/>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input type="email" value={formData.email} onChange={(e) => setFormData(prev => ({ ...prev, email: e.target.value }))} placeholder="email@exemplo.com"/>
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">Telefone</label>
                    <Input type="tel" value={formData.telefone} onChange={(e) => setFormData(prev => ({ ...prev, telefone: e.target.value }))} placeholder="(11) 99999-9999"/>
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">Data de Admissão</label>
                    <Input type="date" value={formData.data_admissao} onChange={(e) => setFormData(prev => ({ ...prev, data_admissao: e.target.value }))}/>
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-700">Turno</label>
                    <Select value={formData.turno} onChange={(e) => setFormData(prev => ({ ...prev, turno: e.target.value }))}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <span className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${(viewingUser.status_usuario === 'ativo' || viewingUser.is_active)
                ? 'bg-green-100 text-green-800'
                : 'bg-red-100 text-red-800'}`}>
                        <span className={`w-2 h-2 rounded-full ${(viewingUser.status_usuario === 'ativo' || viewingUser.is_active) ? 'bg-green-600' : 'bg-red-600'}`}></span>
