...
import { useActivities } from '@/react-app/hooks/useApi';
...
export default function DashboardCollaborator() {
    ...
    const [lancamentosPendentesReprovados, setLancamentosPendentesReprovados] = useState([]);
    ...
    useEffect(() => {
        if (activitiesData) {
            // Ordenar as atividades como era feito antes
            const sortedActivities = [...activitiesData].sort((a, b) => {
                const nameCompare = a.nome_atividade.localeCompare(b.nome_atividade);
                if (nameCompare !== 0)
                    return nameCompare;
                return (b.produtividade_minima || 0) - (a.produtividade_minima || 0);
            });
            setActivities(sortedActivities);
        }
    }, [activitiesData]);
    const calcularNivelAtingido = (nomeAtividade, producaoHora) => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            if (!mesInicializado && historico.length > 0) {
                const datasLancamentos = historico.map((item) => {
                    const dateOnly = item.data_lancamento.split('T')[0];
                    const [year, month, day] = dateOnly.split('-');
                    return new Date(parseInt(year), parseInt(month) - 1, parseInt(day));
                });
                // Encontrar a data mais recente
                const dataMaisRecente = new Date(Math.max(...datasLancamentos.map((d) => d.getTime())));
            ...
            if (responsePendentesReprovados.ok) {
                ...
                const pendentesReprovados = todoLancamentos.filter((item) => item.status === 'pendente' || item.status === 'reprovado');
            ...
            const atividadesPorTipo = dadosUsuario.reduce((acc, item) => {
                ...
                if (userFunction === 'Operador de Empilhadeira') {
                    ...
                    if (dados.tarefas_validas && dados.tarefas_validas > 0) {
                        ...
                        acc[nomeAtividade].dias = new Set(acc[nomeAtividade].historico.map((h) => h.data)).size;
                    }
    ...
    const navegarParaCalculadora = (dia) => {
    ...
    return (<AuthGuard>
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
        ...
        lancamentosPendentesReprovados.filter((item) => {
            // Usar a mesma lógica da formatDateSafe para evitar problemas de timezone
            const dateOnly = item.data_lancamento.split('T')[0];
            const [year, month, day] = dateOnly.split('-');
            const dataLancamento = new Date(parseInt(year), parseInt(month) - 1, parseInt(day));
            ...
            const anoLancamento = dataLancamento.getFullYear();
            return mesLancamento === mesAtual.getMonth() && anoLancamento === mesAtual.getFullYear();
        }).length > 0 ? (<div className="space-y-4 sm:space-y-6">
                ...
                return (<div key={index} className="p-3 sm:p-4 border rounded-lg hover:shadow-md transition-all duration-200 bg-gradient-to-r from-yellow-50 to-amber-50 border-yellow-200">
                                  ...
                                  <div className="flex items-center space-x-3 sm:space-x-4">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <div className="mt-2 sm:mt-3 space-y-1 sm:space-y-2 bg-white p-2 sm:p-3 rounded border">
                                        ...
                                        {/* KPIs Atingidos */}
                                        {kpisAtingidos.length > 0 && (<div className="text-xs sm:text-sm">
                                        ...
                                        {/* Tarefas Válidas */}
                                        {tarefasValidas && (<div className="text-xs sm:text-sm">
                                        ...
                                        {/* Subtotal Atividades */}
                                        {item.subtotal_atividades && (<div className="text-xs sm:text-sm">
        ...
        <Card>
                ...
                <Card>
                  ...
                  <CardContent className="p-3 sm:p-6">
                    <div className="space-y-3 sm:space-y-4">
                      {dashboardData.atividades.map((atividade, index) => (<div key={index} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 bg-gray-50 rounded-lg space-y-3 sm:space-y-0">
                          <div className="flex items-start sm:items-center space-x-3 sm:space-x-4 flex-1">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {userFunction === 'Operador de Empilhadeira' && atividade.nome === 'Tarefas Válidas' && atividade.tarefasValidas && (<div className="mt-2 space-y-1">
                              ...
                              {userFunction === 'Ajudante de Armazém' && atividade.nome !== 'KPIs Atingidos' && (<div className="mt-2 space-y-1">
                                  <p className="text-xs sm:text-sm text-red-600">
                                  ...
                                  {atividade.quantidadeTotalProduzida !== undefined && atividade.quantidadeTotalProduzida > 0 && (<p className="text-xs sm:text-sm text-blue-600 font-medium">
                                      📦 Quantidade Total: {atividade.quantidadeTotalProduzida.toFixed(0)} unidades
                                    </p>)}
                                  {atividade.tempoTotalHoras !== undefined && atividade.tempoTotalHoras > 0 && (<p className="text-xs sm:text-sm text-purple-600">
                                      ⏱️ Tempo Total: {atividade.tempoTotalHoras.toFixed(1)} horas
                                    </p>)}
                                  {atividade.producaoHora !== undefined && atividade.producaoHora > 0 && (<p className="text-xs sm:text-sm text-green-700 font-semibold">
                                      🚀 Produção/h: {atividade.producaoHora.toFixed(1)} unidades/h
                                    </p>)}
                                  {atividade.nivelAtingido && (<p className="text-xs sm:text-sm text-orange-600 font-semibold">
                ...
                {userFunction === 'Ajudante de Armazém' && (<Card>
                    <CardHeader className="p-3 sm:p-6">
                      <CardTitle className="flex items-center space-x-2 text-sm sm:text-base">
                    ...
                    <CardContent className="p-3 sm:p-6">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                        {dashboardData.atividades.map((atividade, index) => (<div key={index} className="bg-gradient-to-br from-white to-gray-50 border-2 border-gray-200 rounded-xl p-3 sm:p-4 hover:shadow-lg transition-all duration-300">
                            ...
                            <div className="space-y-1 text-xs sm:text-sm">
                              ...
                              {atividade.nome !== 'KPIs Atingidos' && (<div className="flex justify-between">
                  ...
                  <CardContent className="p-3 sm:p-6">
                    <div className="space-y-3 sm:space-y-4">
                      ...
                      <div className="grid grid-cols-7 gap-1 sm:gap-2">
                        ...
                        {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((dia, index) => (<div key={index} className="text-center text-xs sm:text-sm font-medium text-gray-600 p-1 sm:p-2">
                            {dia}
                          </div>))}
                ...
                for (let i = 0; i < primeiroDia; i++) {
                    calendario.push(<div key={`empty-${i}`} className="p-1 sm:p-2"></div>);
                }
                // Dias do mês
                for (let dia = 1; dia <= diasDoMes; dia++) {
                    ...
                    calendario.push(<div key={dia} className={`${corFundo} text-white rounded-lg p-1 sm:p-2 text-center transition-all duration-200 relative group ${podeClicar ? 'cursor-pointer hover:opacity-80 hover:scale-105 hover:shadow-lg' : 'cursor-default'}`} title={podeClicar ? `${titulo} - Clique para lançar atividade` : titulo} onClick={podeClicar ? () => navegarParaCalculadora(dia) : undefined}>
                                <div className="text-xs sm:text-sm font-medium">{dia}</div>
                                <div className="text-xs mt-1">{icone}</div>
                                {podeClicar && (<div className="text-xs mt-1 opacity-75">📝</div>)}
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-2 sm:space-y-3 max-h-80 sm:max-h-96 overflow-y-auto">
                      {dashboardData.historicoCompleto.length > 0 ? (dashboardData.historicoCompleto.map((item, index) => {
                // Parse dos dados do lançamento para exibir detalhes
                let kpisAtingidos = [];
                let tarefasValidas = null;
                try {
                ...
                return (<div key={index} className="p-3 sm:p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg hover:shadow-md transition-all duration-200">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2 sm:space-x-4">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="mt-2 sm:mt-3 space-y-1 sm:space-y-2 bg-white p-2 sm:p-3 rounded border">
                                      ...
                                      {userFunction !== 'Operador de Empilhadeira' && item.atividade && item.atividade !== 'KPIs Atingidos' && item.atividade !== 'Tarefas Válidas' && (<div className="text-xs sm:text-sm">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 mt-2 space-y-1 sm:space-y-0">
                                      {item.status_edicao === 'editado_admin' ? (<span className="inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800 w-fit">
                                          ✏️ Aprovado com Edição
                                        </span>) : (<span className="inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 w-fit">
                                          ✅ Aprovado
                                        </span>)}
                                      <span className="text-xs text-gray-500">
                                        👤 {item.status_edicao === 'editado_admin' ? `Editado por: ${item.editado_por_admin || 'Admin'}` : `Aprovado por: ${item.aprovadoPor || 'Sistema'}`}
                                      </span>
                                      {item.status_edicao === 'editado_admin' && item.data_edicao && (<span className="text-xs text-gray-500">
                                ...
                                <div className="text-right">
                                  <div className="space-y-1">
                                    ...
                                    {((item.bonus_kpis || 0) > 0 || (item.subtotal_atividades || 0) > 0) && (<div className="text-xs text-gray-600 space-y-0.5">
                                        {(item.subtotal_atividades || 0) > 0 && (<div>📋 Atividades: R$ {valorAtividades.toFixed(2)}</div>)}
                                        {(item.bonus_kpis || 0) > 0 && (<div>🎯 KPIs: R$ {valorKpis.toFixed(2)}</div>)}
                ...
                dashboardData.historicoCompleto.reduce((total, item) => total + (item.bonus_kpis || 0), 0).toFixed(2) : '0.00'}
                      ...
                      {/* Tarefas Válidas */}
                      {userFunction === 'Operador de Empilhadeira' && (<div className="bg-white p-3 sm:p-4 rounded-lg border border-purple-200">
                          <div className="flex items-center justify-between">
                            <div>
                    ...
                    dashboardData.historicoCompleto.reduce((total, item) => total + (item.valor_bruto_atividades || item.valor_tarefas || 0), 0).toFixed(2) : '0.00'} | Líquido: R$ {dashboardData.historicoCompleto ?
                    dashboardData.historicoCompleto.reduce((total, item) => total + (item.subtotal_atividades || 0), 0).toFixed(2) : '0.00'}
                      ...
                      {userFunction === 'Ajudante de Armazém' && (<div className="bg-white p-3 sm:p-4 rounded-lg border border-indigo-200">
                          <div className="flex items-center justify-between">
                            <div>
                    ...
                    }, 0).toFixed(2) : '0.00'} | Líquido: R$ {dashboardData.atividades.reduce((total, ativ) => {
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-2 sm:space-y-3 max-h-80 sm:max-h-96 overflow-y-auto">
                      {lancamentosPendentesReprovados.length > 0 ? (lancamentosPendentesReprovados
                .filter((item) => {
                // Usar a mesma lógica da formatDateSafe para evitar problemas de timezone
                const dateOnly = item.data_lancamento.split('T')[0];
                const [year, month, day] = dateOnly.split('-');
                ...
                return (<div key={index} className={`p-3 sm:p-4 border rounded-lg hover:shadow-md transition-all duration-200 ${isPendente ? 'bg-gradient-to-r from-yellow-50 to-amber-50 border-yellow-200' :
                        'bg-gradient-to-r from-red-50 to-rose-50 border-red-200'}`}>
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                                <div className="flex items-center space-x-3 sm:space-x-4">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="mt-2 sm:mt-3 space-y-1 sm:space-y-2 bg-white p-2 sm:p-3 rounded border">
                                      ...
                                      {/* Atividade principal */}
                                      {item.nome_atividade && item.nome_atividade !== 'KPIs Atingidos' && (<div className="text-xs sm:text-sm">
                                          <span className="font-medium text-orange-600">🏃‍♂️ Atividade:</span>
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 mt-2 space-y-1 sm:space-y-0">
                                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${isPendente ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                                        {isPendente ? '⏳ Pendente' : '❌ Reprovado'}
                                      </span>
                                      {item.aprovadoPor && (<span className="text-xs text-gray-500">
                                          👤 {isReprovado ? 'Reprovado' : 'Analisado'} por: {item.aprovadoPor}
                                        </span>)}
                                      {isReprovado && item.motivo_reprovacao && (<span className="text-xs text-red-600">
                                ...
                                <div className="text-center sm:text-right">
                                  <p className={`text-lg sm:text-2xl font-bold ${isPendente ? 'text-yellow-600' : 'text-red-600'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <Card className="bg-gradient-to-r from-yellow-50 to-orange-50 border-yellow-200">
                    ...
                    <CardContent>
                      {dashboardData.melhorDia.data ? (<div className="space-y-1 sm:space-y-2">
                        ...
                        </div>) : (<p className="text-sm sm:text-base text-yellow-700">Sem dados disponíveis</p>)}
                    </CardContent>
                  </Card>
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        <div>
                          ...
                          <div className="w-full bg-indigo-200 rounded-full h-2 sm:h-3">
                            <div className="bg-gradient-to-r from-indigo-400 to-purple-600 h-2 sm:h-3 rounded-full transition-all duration-300" style={{ width: `${Math.min(dashboardData.percentualMeta, 100)}%` }}></div>
                          </div>
