...
export default function UserManagement({ users, onAddUser, onUpdateUser, onDeleteUser, loading }) {
              ...
              <Table>
                ...
                <TableBody>
                  {users.map((user) => (<TableRow key={user.id}>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          <div className="flex items-center space-x-2">
                            {user.tipo_usuario === 'admin' ? (<Shield className="h-4 w-4 text-purple-600"/>) : (<User className="h-4 w-4 text-gray-600"/>)}
                      ...
                      <TableCell>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${user.status_usuario === 'ativo'
                    ...
                    : 'bg-red-100 text-red-800'}`}>
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          <Button variant="outline" size="sm" onClick={() => openEditDialog(user)} title="Editar usuário">
                            <Edit className="h-3 w-3"/>
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => toggleUserRole(user)} className={user.tipo_usuario === 'admin' ? 'text-purple-600' : 'text-gray-600'} title={`Alterar perfil para ${user.tipo_usuario === 'admin' ? 'Colaborador' : 'Administrador'}`}>
                            <Shield className="h-3 w-3"/>
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => toggleUserStatus(user)} className={user.status_usuario === 'ativo' ? 'text-red-600' : 'text-green-600'} title={user.status_usuario === 'ativo' ? 'Desativar usuário' : 'Ativar usuário'}>
                            {user.status_usuario === 'ativo' ? <UserX className="h-3 w-3"/> : <UserCheck className="h-3 w-3"/>}
                          </Button>
                          <Button variant="destructive" size="sm" onClick={() => handleDelete(user)} title="Excluir usuário">
        ...
        <DialogContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={formData.nome} onChange={(e) => setFormData(prev => ({ ...prev, nome: e.target.value }))} placeholder="Nome completo" required/>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <input type="checkbox" id="status_usuario" checked={formData.status_usuario === 'ativo'} onChange={(e) => setFormData(prev => ({ ...prev, status_usuario: e.target.checked ? 'ativo' : 'inativo' }))} className="rounded border-gray-300"/>
