...
import { ActivitySchema, KPISchema, CalculatorInputSchema, UserSchema, LoginSchema, CreateLancamentoSchema, AdminValidationSchema, ExportFilterSchema, KPILimitCheckSchema } from "../../src/shared/types";
...
app.post('/api/calculate', zValidator('json', CalculatorInputSchema), async (c) => {
    ...
    try {
        ...
        if (valor_bruto_atividades_calculated !== null)
            result.valor_bruto_atividades = valor_bruto_atividades_calculated;
        ...
        for (const turno of turnos) {
            for (const funcao of funcoes) {
                ...
                for (const lancamento of lancamentosFuncao) {
                    if (Array.isArray(lancamento.atividades) && lancamento.atividades[0]?.meta_tempo && lancamento.tempo_execucao) {
                        const eficiencia = (lancamento.atividades[0].meta_tempo / lancamento.tempo_execucao) * 100;
